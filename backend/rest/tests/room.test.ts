...
describe("/room", () => {
  ...
  it("Should create user and room", async () => {
    ...
    expect(resGuest.body.username).toBeDefined();
    expect(resGuest.body.token).toBeDefined();
    expect(resGuest.body.userId).toBeDefined();
    userId = resGuest.body.userId;
  ...
  it("Should not create room with invalid values", async () => {
    ...
    let res = await request(app).post("/room").send(body);
    expect(res.statusCode).toEqual(400);
    expect(res.body.error).toBe("Invalid max player count");
    body.maxPlayers = 10;
    res = await request(app).post("/room").send(body);
    expect(res.statusCode).toEqual(400);
    expect(res.body.error).toBe("Invalid max player count");
    body.maxPlayers = 2;
    body.pinCode = "2";
